 ...
 */
 ...
 function getFileHash($tid)
 {
  ...
  if (file_exists(JPATH_COMPONENT_ADMINISTRATOR_ONEPAGE.DS.'export'.DS.'templates'.DS.$t['file']))
  $mdate = filemtime(JPATH_COMPONENT_ADMINISTRATOR_ONEPAGE.DS.'export'.DS.'templates'.DS.$t['file']);
 ...
 function getTxtTemplate($tid)
 {
  ...
  $tfile = JPATH_ADMINISTRATOR.DS.'components'.DS.'com_onepage'.DS.'export'.DS.'templates'.DS.$tid['file'];
 ...
 function processPhpTemplate($tid, $localid, &$data)
 {
   ...
   $tfile = JPATH_ADMINISTRATOR.DS.'components'.DS.'com_onepage'.DS.'export'.DS.'templates'.DS.$tidd['file'];
   if (file_exists($tfile))
   {
 ...
 function getExportTemplates($type = 'ORDER_DATA')
{
 ...
 $path = JPATH_COMPONENT_ADMINISTRATOR_ONEPAGE.DS.'export'.DS.'templates';
 $maxs = 0;
 $files = scandir($path);
 $reta = array();
 foreach ($files as $f)
 {
  ...
  if ($f != '.' && $f != '..' && (strpos($f, '.ht')===false) && (!is_dir($path.DS.$f)))
  {
   $reta[] = $f;
  }
 }
...
//	 $xml .= '<LOCALID>'.$localid.'</LOCALID>';
   ...
   $config =& JFactory::getConfig();
    ...
    function listExports2()
    {
	 ...
	 <?php
	  ...
	  ?><div id="tiddiv_<?php echo $t['tid'].'_'.$k['localid']; ?>_NONE" <?php if ($status != 'NONE') echo ' style="display: none;" '; ?>><?php
 	  echo $lin.'<img src="'.$mosConfig_live_site.'/components/com_onepage/assets/images/new.png" alt="'.$status_txt.'" title="'.$status_txt.'" /></a>';
	  ?></div><?php
    ...
    function getExportHtml($noheader = false)
    {
     ...
     global $mosConfig_live_site;
	  ...
	  <div id="tiddiv_{template_id}_{local_id}_NONE" {style_none}>
 	  	...
 	  	<img src="'.$mosConfig_live_site.'/components/com_onepage/assets/images/new.png" alt="CREATE" title="CREATE" />
 	  	</a>
	  </div>
	...
	function updateFileName($tid, $fname)
	{
	 ...
	 $q = "select tid from #__onepage_export_templates where file = '".$dbj->getEscaped(urlencode($fname))."' limit 0,1"; 
	 $dbj->setQuery($q); 
	 $tid2 = $dbj->loadResult(); 
	 ...
	 if ($tid2 != $tid)
	  {
	      if (file_exists(JPATH_COMPONENT.DS.'export'.DS.'templates'.DS.$fname))
		  {
		    return 'Another theme is using this filename !'; 
		  }
		  else
		  {
		    $q = "delete from #__onepage_export_templates where file = '".$dbj->getEscaped(urlencode($fname))."' limit 1"; 
			$dbj->setQuery($q); 
			$dbj->query(); 
    ...
    {
     $t = $this->getTemplate($tid);
     $file = JPATH_COMPONENT_ADMINISTRATOR_ONEPAGE.DS.'export'.DS.'templates'.DS.$t['file'];
...
function getOrderDataEx($tid, $localid, $special_value = null, $ind=0)
{
 ...
 foreach ($data as $key2=>$r2)
 {
  ...
  if (strpos($key2, 'cdate')!==false)
  {
   ...
   $data[$key2.'_vmdate'] = $date->toFormat(); //vmFormatDate( $data[$key2] + $mosConfig_offset );
   $data[$key2.'_iso'] = date("Y-m-d",$data[$key2] + $mosConfig_offset);
  }
  
 }
 $now = JFactory::getDate(time()); 
 $data['export_created_date_unix'] = time();
 $data['export_created_date_vm'] = $now->toFormat(); //vmFormatDate(time()+$mosConfig_offset, $VM_LANG->_('DATE_FORMAT_LC'));
