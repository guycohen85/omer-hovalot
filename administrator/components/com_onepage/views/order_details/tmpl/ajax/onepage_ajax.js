function op_runCmd(cmd, element, extradata)
{
 if ((typeof(deb) == 'undefined') || (deb == null))
  deb = document.getElementById('debug_window');
 if (deb != null)
 {
  deb.style.display = '';
 }
 ...
 if (cmd == 'orderstatusset')
 {
  ...
  order_statush = document.getElementById('order_status');
  order_status = order_statush.options[order_statush.selectedIndex].value;
  q = '&notify_customer='+notify_customer+'&include_comment='+include_comment+'&order_comment='+order_comment+'&current_order_status='+current_order_status+'&order_status='+order_status+'&cmd=OrderStatusSet';
  // order_item_id
  // order_number
 ...
 // alert('multioff');
  ...
  else
  {
   ...
   else
   {
    ...
    d.innerHTML = '<div id="eid_'+data[5]+'" >'+data[6]+'</div><br style="clear: both;"/>'+d.innerHTML;
    //alert('row added');
   // alert('som tu'+data[5]); 
...
function op_get_SS_response()
{
  ...
  if (xmlhttp2.readyState==4 && xmlhttp2.status==200)
    ...
    timeOut=setTimeout("hide_debug()",20000);
    return true;
}
