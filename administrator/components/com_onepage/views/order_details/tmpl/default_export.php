	...
	foreach ($templates as $t)
	{
	 ...
	 <?php
	  ...
	  ?><div id="tiddiv_<?php echo $t['tid'] ?>_NONE" <?php if ($status != 'NONE') echo ' style="display: none;" '; ?>><?php
 	  echo '<a href="'.$this->ehelper->getExportItemLink($t['tid'], $order_id).'" id="tid_'.$t['tid'].'_none" onclick="'."javascript:return op_runCmd('sendXml', this);".'" >'.'<img src="'.Juri::root(true).'components/com_onepage/assets/images/new.png" alt="'.$status_txt.'" title="'.$status_txt.'" /></a>';
	  ?></div><?php
	  $item = $this->ehelper->getExportItem($t['tid'], $order_id);
	  $link = $this->ehelper->getPdfLink($item);
	  if (empty($link)) $link = '#';
	  $created_html = '<a href="'.$link.'" id="tidpdf_'.$t['tid'].'" target="_blank"'." ><img src='".Juri::root(true)."components/com_onepage/assets/images/pdf_button.png' alt='".$status_txt."' title='".$status_txt."' />".'</a>';
	  $processing_html2 = '<a href="#" id="tid_'.$t['tid'].'_2" onclick="javascript:return op_runCmd('."'sendXml'".', this);"'." ><img src='".Juri::root(true)."components/com_onepage/assets/images/process.png' alt='RECREATE' title='RECREATE' /></a>";
	  $email = '   <a href="#" id="tid_'.$t['tid'].'_email" onclick="javascript:return op_runCmd('."'sendEmail'".', this);"><img src="'.Juri::root(true).'components/com_onepage/assets/images/emailButton.png" /></a>';
