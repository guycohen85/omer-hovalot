...
defined('_JEXEC') or die('Restricted access');
	...
	if(function_exists("wp_register_style") == false){
		 function wp_register_style($handle, $url = null){
			...
			if(empty($url))
				UniteFunctionsUG::throwError("wp register style error: empty url. handle: {$handle}");
	...
	if(function_exists("wp_register_script") == false){
		 function wp_register_script($handle, $url){
			...
			if(empty($url))
				UniteFunctionsUG::throwError("wp register script error: empty url. handle: {$handle}");
