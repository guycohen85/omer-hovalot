...
class plgSystemAdminExile extends JPlugin {
    ...
    private function _blackwhite($ip) {
        ...
        foreach (array('whitelist', 'blacklist') as $list) {
            $listnet = $list . 'net';
            $listdefault = ($list == 'blacklist') ? JText::_('PLG_SYS_ADMINEXILE_DEFAULT_BLACKLIST') : JText::_('PLG_SYS_ADMINEXILE_DEFAULT_WHITELIST');
        ...
        if (count(array_merge($whitelistnet, $blacklistnet))) {
            ...
            if(!class_exists($requires[$require])) require_once(__DIR__.'/classes/'.$require.'.class.php');
