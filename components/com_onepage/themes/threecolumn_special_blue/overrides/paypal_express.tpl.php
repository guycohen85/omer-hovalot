...
$ppex = JRequest::getVar('ppex_cancel', '');
$ppex_gecd = JRequest::getVar('ppex_gecd', ''); 
if($_REQUEST['page']=='shop.cart' && ((!empty($ppex)) || (empty($ppex_gecd))))
{
...
if (!defined('_MIN_POV_REACHED')) { ?>
    <span style="font-weight:bold;"><?php echo OPCLang::_('COM_VIRTUEMART_CHECKOUT_ERR_MIN_POV2') . " ".$CURRENCY_DISPLAY->getFullValue($_SESSION['minimum_pov']) ?></span>
<?php } else {
    if ($_REQUEST['page']=='shop.cart' && (PAYPAL_API_CART_BUTTON_ON == '1' || PAYPAL_API_CART_BUTTON_ON == 1)) {
        $paypal_express_href = $sess->url( $_SERVER['PHP_SELF'].'?page=checkout.onepage&ssl_redirect=1&ppex=1', true);
