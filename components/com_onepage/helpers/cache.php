...
class OPCcache {
	...
	public static function storeShipingCalculation(&$cart, &$cart_prices, &$cart_prices_name, $idth) 
	{
	  ...
	  require(JPATH_SITE.DS.'components'.DS.'com_onepage'.DS.'helpers'.DS.'third_party'.DS.'third_party_request_cache.php'); 
	...
	function getGeneralCacheHash($name, &$cart, &$data=array())
	{
	   ...
	   foreach ($returnValues as $val)
	   {
		...
		if (!isset(OPCloader::$debugMsg)) OPCloader::$debugMsg = array(); 
	...
	public static function installCache()
	{
		...
		$path = JPATH_ROOT.DS.'cache'.DS.'com_onepage'; 
		if (!file_exists($path))
		{
		...
		{
			$data = ' '; 
			if (@JFile::write($path.DS.'index.html', $data)==false)
				define('OPCCACHEINSTALLED', false); 
		}
	...
	public static function get($hash)
	{
		...
		$md = $hash; 
		if (!file_exists(OPCCACHEPATH.DS.$md.'_timeout.php')) return null; 
		include(OPCCACHEPATH.DS.$md.'_timeout.php'); 
		$time = time(); 
		...
		if ((!empty($timeout)) && ($time <= $timeout))
		{
			...
			include(OPCCACHEPATH.DS.$md.'.php'); 
		...
		{
			jimport( 'joomla.filesystem.file' );
			JFile::delete(OPCCACHEPATH.DS.$md.'_timeout.php'); 
			JFile::delete(OPCCACHEPATH.DS.$md.'.php'); 
		}
...
'; 
		...
		if (@JFile::write($path.DS.$hash.'.php', $data)==false)
			{
...
$timeout = '.$time.'; 
'; 
				if (@JFile::write($path.DS.$hash.'_timeout.php', $data2)==false)
				{
					define('OPCCACHEPROBLEM', false); 
